...
-- Approach:-
-- We subtract the row number from the log_id.
-- For continuous numbers (e.g., 1, 2, 3), this difference will be the same, creating a group.
...
Objective: Find employees who have the highest salary in each department. If there are multiple employees in a department with the same highest salary, include all of them.
...
/* 
-- Approach:
For each department (PARTITION BY departmentId), we assign a rank to employees based on salary.
DENSE_RANK() gives 1 to the highest salary (even if there are ties).
We then select only those with rank = 1 (highest salary).
Finally, we join with the Department table to get department names.
*/
...
Approach:
------------
First, for each department, find the maximum salary using GROUP BY.
Then, in the outer query, select employees who match those (departmentId, salary) pairs.
Simple, and works even if there are multiple top earners in a department.
*/
/*
...
Approach:
-----------
Calls can go in either direction (e.g., 1→2 or 2→1), but we want to treat both as the same pair.
LEAST(from_id, to_id) and GREATEST(from_id, to_id) ensure:
(1, 2) and (2, 1) → both treated as (1, 2)
Then we group by the person pairs, count how many calls, and sum the durations.
*/
